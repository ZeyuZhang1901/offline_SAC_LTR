...
def extend_and_repeat(tensor, dim, repeat):
    # Extend and repeat the tensor along dim axie and repeat it
...
class ReparameterizedTanhGaussian(nn.Module):
    ...
    def forward(self, mean, log_std, deterministic=False):
        ...
        # change action sample into probs (be careful with Nan)
        exp_action_sample = torch.exp(action_sample)
        sum = torch.sum(exp_action_sample,dim=1,keepdim=True)
        action_sample = exp_action_sample/sum
        nan_solve = torch.isnan(action_sample) * torch.ones_like(action_sample)/\
            torch.sum(torch.ones_like(action_sample),dim=1,keepdim=True)
        action_sample = torch.where(torch.isnan(action_sample),\
            torch.zeros_like(action_sample),action_sample)
        action_sample += nan_solve
