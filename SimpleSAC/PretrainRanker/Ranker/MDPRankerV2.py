...
class MDPRankerV2(AbstractRanker):
    def __init__(self,
        ...
        )  # used for multiprocessor training, otherwise has errors
        ...
        # scores = input_docs * aW1 + b1
        self.aW1 = tf.Variable(
            tf.truncated_normal([self.Nfeature, 1],
                                stddev=0.1 / np.sqrt(float(self.Nfeature))))
        # b1 = tf.Variable(tf.zeros([1, hidden_units]))
        ah1 = tf.matmul(self.input_docs, self.aW1)
        self.doc_scores = tf.transpose(ah1)  # dim = 1 * num_doc

        if loss_type == 'pointwise':
            self.prob = tf.nn.softmax(self.doc_scores)  # prob of each doc to be clicked

            neg_log_prob = tf.reduce_sum(
                -tf.log(tf.clip_by_value(self.prob, 1e-10, 1.0)) *
                tf.one_hot(self.position, self.doc_length),
                axis=1)
            self.loss = tf.reduce_mean(neg_log_prob * self.advantage)

        if loss_type == 'pairwise':
            self.position2 = tf.placeholder(tf.int64)
            self.exp = tf.math.exp(
                tf.clip_by_value(
                    self.doc_scores[0][self.position] -
                    self.doc_scores[0][self.position2], -100, 70))
            self.P = tf.math.divide(self.exp, (1 + self.exp))

            neg_log_prob = tf.reduce_sum(
                -tf.log(tf.clip_by_value(self.P, 1e-10, 1.0)) *
                tf.one_hot([0], self.doc_length),
                axis=1)
        ...
        self.accum_ops = [
            accum_vars[i].assign_add(gv[0]) for i, gv in enumerate(self.gvs)
        ]
        self.actor_train_step = self.train_op.apply_gradients([
            (accum_vars[i], gv[1]) for i, gv in enumerate(self.gvs)
        ])  # gv[1] is equal to the current tvs
    ...
    def update_policy(self, query, ranklist, rewards, dataset):
        ...
        self.sess.run(self.zero_ops)  # gradient clear to zero
            ...
            for pos in range(lenghth):
                ...
                gradient = self.sess.run(
                    ...
                    feed_dict={
                        ...
                        self.doc_length: len(ranklist[pos:]),
                        self.advantage: rewards[pos]
                    })[0][0][0]  # get gradient as np.array
    ...
    def get_query_result_list(self, dataset, query):
        ...
        for position in range(ndoc):
            ...
            choice = np.random.choice(len(probabilities), 1,
                                      p=probabilities)[0]
    ...
    def get_all_query_result_list(self, dataset):
        ...
        for query in dataset.get_all_querys():
            ...
            docid_score_list = np.flip(
                docid_score_list[docid_score_list[:, 1].argsort()], 0)
    ...
    def get_scores(self, features):
        ...
        result = self.sess.run([self.doc_scores],
                               feed_dict={self.input_docs:
    ...
    def save_model(self, path, globalstep, write_graph=False):
        self.saver.save(self.sess,
                        path,
                        global_step = globalstep,
                        write_meta_graph= write_graph)
